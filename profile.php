...
<body class="font-sans">
    ...
    <nav x-data="{ menuOpen: false }" class="flex justify-between items-center py-4 px-6 border-b shadow-md relative">
        ...
        <section class="p-8 border-b">
            ...
            <div class="flex justify-center space-x-4">
                ...
                <a href="change_password.php" class="bg-gray-500 text-white px-4 py-2 hover:bg-gray-400 hover:text-white-600 rounded-md">Change Password</a>

                <a href="calendar.php" class="bg-blue-500 text-white px-4 py-2 hover:bg-blue-400 hover:text-white-600 rounded-md">Google Calendar</a>

                <form action="delete_account.php" method="GET">
                    <button type="submit" class="bg-red-500 text-white px-4 py-2 hover:bg-red-700 hover:text-white-600 rounded-md">Delete Account</button>
                </form>
            </div>
        </section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <?php foreach ($favoriteMeals as $meal): ?>
            <div class="recipe-card border p-4 w-64 mb-4 shadow-2xl ml-20 md:ml-30 rounded-lg overflow-hidden bg-white" data-mealid="<?= $meal['idMeal'] ?>">
                <!-- Image Section -->
                ...
                <!-- Ratings Section -->
                <div id="rating-info-<?= $meal['idMeal'] ?>" class="text-center mt-2 text-sm text-gray-600">Loading rating...</div>
                <div class="flex flex-col mt-2">
                    <input type="number" min="1" max="5" step="0.1" id="rating-<?= $meal['idMeal'] ?>" placeholder="Rate (1-5)" class="border p-2 mb-2 rounded-md w-full" />
                    <button onclick="rateMeal('<?= $meal['idMeal'] ?>')" class="border p-2 bg-yellow-400 hover:bg-yellow-300 rounded-md w-full">
                ...
                <div class="flex justify-between mt-2">
                    <button onclick="deleteMeal('<?= $meal['idMeal'] ?>')" class="border p-2 bg-red-400 hover:bg-red-300 rounded-md flex-1 mr-1">
                        <i class="fas fa-trash"></i> Delete
                    </button>
                    <button onclick="bookmarkMeal('<?= $meal['idMeal'] ?>')" class="border p-2 bg-blue-400 hover:bg-blue-300 rounded-md flex-1 ml-1">
                        <i class="fas fa-calendar"></i> Bookmark
                    </button>
                </div>

                <!-- View Recipe Button -->
                <button onclick="showRecipeDetails('<?= $meal['idMeal'] ?>')" class="border p-2 w-full mt-2 bg-gray-200 hover:bg-gray-100 rounded-md">
                    View Recipe
                </button>
            </div>
        <?php endforeach; ?>
